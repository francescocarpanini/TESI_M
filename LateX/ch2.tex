...
\begin{itemize}
    ...
    \end{itemize}
    A visual representation of the multi-channel tensor is presented in Figure \ref{fig:bev-loader}, where each layer is represented with a distinct color, illustrating the layered structure of the tensor.
    \begin{figure}[H]
        \centering
    ...
    \item \textit{Ego Loader:} This component represents the ego vehicle within the same tensor used for other sensor data. A dedicated function generates a rectangular outline corresponding to the vehicle's physical dimensions. These dimensions are derived from the actual vehicle used to record the dataset sequences, ensuring accurate representation and alignment within the scene.
    ...
    This method ensures smooth and precise boundary representation, as illustrated in Figure \ref{fig:boundary-rendering}.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.5\linewidth]{LateX//figs/polyline.png}
        \caption{Example of Boundaries Rendered with Polylines}
        \label{fig:boundary-rendering}
    \end{figure}
...
Specifically, we focus on transformations between these two frames:
\begin{enumerate}
